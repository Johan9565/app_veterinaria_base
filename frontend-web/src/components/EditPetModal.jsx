...
const EditPetModal = ({ pet, onClose, onSubmit }) => {
  ...
  // Cargar clientes (usuarios con rol cliente y owner)
  useEffect(() => {
  ...
  // Asegurar que el propietario se establezca correctamente cuando se carguen los clientes
  useEffect(() => {
    if (clients.length > 0 && pet && selectedOwner) {
     
      const ownerExists = clients.find(client => client.value === selectedOwner);
      if (!ownerExists) {
  ...
  }, [clients, pet, selectedOwner, formData.species, formData.gender]);

  // Poblar formulario con datos de la mascota
  useEffect(() => {
    if (pet) {
      ...
      const formDataToSet = {
        ...
        neuteredDate: pet.neuteredDate ? new Date(pet.neuteredDate).toISOString().split('T')[0] : '',
        healthStatus: pet.healthStatus || 'bueno',
  ...
  const handleChange = (field, value) => {
    ...
    if (shouldUpperCase && typeof value === 'string') {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Remover fecha de esterilización si no está esterilizado
      if (!petData.isNeutered) {
        delete petData.neuteredDate;
      }

      await onSubmit(pet._id, petData);
      AlertService.success('Mascota actualizada', 'La mascota ha sido actualizada exitosamente');
      onClose();
    } catch (err) {
      console.error('Error actualizando mascota:', err);
      AlertService.error('Error', err.message || 'No se pudo actualizar la mascota');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b border-[#E2E8F0] bg-[#F5F5F5]">
          <h2 className="text-xl font-semibold text-gray-900">Editar Mascota: {pet.name}</h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              <h3 className="text-lg font-medium text-gray-900 border-b border-[#A8E6CF] pb-2">
                Información Básica
              </h3>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                />
                                 <Select2
                   key={`weight-unit-${formData.weight.unit}`}
                   label="Unidad"
                   options={[
            ...
            <div className="space-y-4">
              <h3 className="text-lg font-medium text-gray-900 border-b border-[#A8E6CF] pb-2">
                Información Médica
                             ...
                             <Select2
                 key={`health-status-${formData.healthStatus}`}
          ...
          {/* Botones */}
          <div className="flex justify-end space-x-3 mt-6 pt-6 border-t border-[#E2E8F0] bg-[#F5F5F5] p-4 -mx-6 -mb-6">
            <Button
              type="button"
              variant="outline"
              onClick={onClose}
